...
function isFilesystemRootDotenv(envPath) {
...
if (fs.existsSync(parentEnv) && !isFilesystemRootDotenv(parentEnv)) {
    dotenv_1.default.config({ path: parentEnv });
}
...
if (fs.existsSync(backendEnv)) {
    ...
    dotenv_1.default.config({ path: backendEnv, override: process.env.DOCKER !== '1' });
}
...
const song_list_1 = require("./song-list");
...
if (!fs.existsSync(PPT_LIBRARY_PATH)) {
    const skipMkdir = process.env.DOCKER === '1' &&
        (PPT_LIBRARY_PATH.startsWith('/app/cloud') || PPT_LIBRARY_PATH.includes(`${path.sep}app${path.sep}cloud`));
    if (skipMkdir) {
        console.warn(`⚠️ Docker: PPT 路徑不存在（請在專案根 .env 設定 SONGS_CLOUD_ROOT=你的雲端資料夾，與 Z 槽同層）: ${PPT_LIBRARY_PATH}`);
    }
    else {
...
function directoryContainsAnyScoreFile(rootDir, depth = 0) {
    if (depth > 30)
        return false;
    const scoreExt = (n) => {
    ...
    try {
        ...
        for (const name of names) {
            ...
            if (st.isFile() && scoreExt(name))
                return true;
            if (st.isDirectory() && directoryContainsAnyScoreFile(full, depth + 1))
                return true;
        }
    }
    catch {
...
function resolveScoreLibraryPath() {
    const tryDir = (p) => {
    ...
    if (env) {
        ...
        console.warn(`⚠️ SCORE_LIBRARY_PATH 指向的目錄不存在: ${path.resolve(env)}（請確認 SONGS_CLOUD_ROOT 已掛到 /app/cloud 且含「2025 別是巴教會雲端詩歌譜」資料夾）`);
        push(env);
    }
    push(path.join(PPT_LIBRARY_PATH, '2025 別是巴教會雲端詩歌譜'));
    push(path.join(PPT_LIBRARY_PATH, '2025 別是巴聖教會雲端詩歌譜'));
    try {
        const songFile = (0, song_list_1.resolveSongListFilePath)();
...
app.post('/api/login', async (req, res) => {
    ...
    // 將 role 寫入 token
    const token = jsonwebtoken_1.default.sign({ id: user._id, role: user.role || 'admin' }, JWT_SECRET, { expiresIn: '24h' });
...
app.put('/api/users/:id', authenticateToken, requireSuperAdmin, async (req, res) => {
    ...
    await db.collection(USERS_COLLECTION_NAME).updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: updateData });
...
app.delete('/api/users/:id', authenticateToken, requireSuperAdmin, async (req, res) => {
    ...
    const db = dbClient.db(DB_NAME);
    await db.collection(USERS_COLLECTION_NAME).deleteOne({ _id: new mongodb_1.ObjectId(id) });
    res.json({ message: 'User deleted' });
});
// --- Song & File Routes ---
const MAX_SONGS_LIST_LIMIT = 50000;
function parseSkipFileStatus(req) {
    const q = req.query;
...
app.get('/api/songs', async (req, res) => {
    try {
        ...
        const all = (0, song_list_1.loadSongRecordsFromFile)();
        const { slice, total, totalPages } = (0, song_list_1.getSongListPage)(all, page, limit, name);
        if (skipFileStatus) {
        ...
        const songsWithStatus = await Promise.all(slice.map(async (song) => {
            const filePath = await (0, generator_1.findPptPath)(PPT_LIBRARY_PATH, song);
            let hasScore = false;
            if (SCORE_LIBRARY_PATH && fs.existsSync(SCORE_LIBRARY_PATH)) {
                const scorePath = await (0, generator_1.findScorePath)(SCORE_LIBRARY_PATH, song);
                hasScore = !!scorePath;
            }
...
app.get('/api/songs/:id/download', async (req, res) => {
    try {
        ...
        if (isScore) {
            ...
            filePath = await (0, generator_1.findScorePath)(SCORE_LIBRARY_PATH, song);
        }
        else {
            filePath = await (0, generator_1.findPptPath)(PPT_LIBRARY_PATH, song);
        }
...
app.post('/api/preview', async (req, res) => {
    ...
    try {
        const data = await (0, generator_1.extractSongData)(songs, PPT_LIBRARY_PATH);
        res.json(data);
    }
    catch (e) {
...
app.post('/api/generate', async (req, res) => {
    ...
    try {
        ...
        const zipPath = await (0, generator_1.generateFiles)(input);
...
const songListFileRefreshHandler = (_req, res) => {
    try {
        (0, song_list_1.clearSongFileCache)();
...
app.listen(port, () => {
    ...
    if (SCORE_LIBRARY_PATH) {
        ...
        console.log(`Score library: (歌譜根目錄解析失敗，環境變數為 ${process.env.SCORE_LIBRARY_PATH}，請檢查 SONGS_CLOUD_ROOT 掛載)`);
    }
    else {
